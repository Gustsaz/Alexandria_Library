...
function applyTheme(theme) {
    if (theme === "dark") {
        ...
        // troca a cor do ícone Font Awesome de conta
        const userIcons = document.querySelectorAll('.fa-user');
        userIcons.forEach(icon => icon.style.color = '#fff');
    } else {
        ...
        // troca a cor do ícone Font Awesome de conta
        const userIcons = document.querySelectorAll('.fa-user');
        userIcons.forEach(icon => icon.style.color = '#333');
    }

    // Atualiza ícone de usuário conforme tema
...
function populateBooks(books) {
    ...
    // Preenche livros
    books.forEach(livro => {
        const categoria = (livro.categoria || 'Todos').trim().toLowerCase();
...
function populateCategories(books) {
    ...
    const categoryCount = {};
    books.forEach(livro => {
        const cat = (livro.categoria || 'Todos').trim().toLowerCase();
        categoryCount[cat] = (categoryCount[cat] || 0) + 1;
    });

    const categories = [
        { name: 'Todos', key: 'todos', count: books.length },
        { name: 'Aventura', key: 'aventura', count: categoryCount['aventura'] || 0 },
        { name: 'Fantasia', key: 'fantasia', count: categoryCount['fantasia'] || 0 },
        { name: 'Romance', key: 'romance', count: categoryCount['romance'] || 0 },
        { name: 'Suspense', key: 'suspense', count: categoryCount['suspense'] || 0 },
        { name: 'Scifi', key: 'scifi', count: categoryCount['scifi'] || 0 },
        { name: 'Terror', key: 'terror', count: categoryCount['terror'] || 0 },
        { name: 'Quadrinho', key: 'quadrinho', count: categoryCount['quadrinho'] || 0 },
        { name: 'Gutenberg', key: 'gutenberg', count: 20 }
    ];
    ...
    categoriesContainer.innerHTML = categories.map(cat => `
        <button class="category ${cat.name === 'Todos' ? 'active' : ''}"
            data-category="${cat.key}">
            <img draggable="false" src="img/icons/${iconMap[cat.key]}" alt="${cat.name}" 
                 style="width: 70px; height: 70px;">
            ${cat.name} (${cat.count})
        </button>
    `).join('');

    // Reaplica comportamento de troca com animação
    const fadeSections = document.querySelectorAll('.highlight[data-category]');
            ...
            fadeSections.forEach(section => {
                ...
                setTimeout(() => {
                    section.style.display =
                        selected === 'todos' || section.dataset.category.toLowerCase() === selected
                            ? 'block' : 'none';
                    section.style.opacity = 1;
                }, 200);
            });

            if (selected === 'gutenberg') loadGutenbergBooks();
        });
    });
}
...
window.onload = function () {
    ...
    // Ativar automaticamente o botão "Todos", mas mostrar todas as seções
    const todosBtn = document.querySelector('.category[data-category="Todos"]');
    if (todosBtn) todosBtn.classList.add('active');

    // Exibir todas as seções de livros
...
function updateUserInterface(user) {
    ...
    <button class="user-btn" id="logoutBtn">
        <i class="fa-solid fa-user user-icon" style="font-size: 22px; margin-right: 8px;"></i>
    </button>
...
// Reaplica o comportamento de clique nas categorias
        ...
        document.querySelectorAll('.highlight[data-category]').forEach(section => {
            section.style.display =
                categoriaSelecionada === "Todos" ||
                    section.dataset.category.toLowerCase() === categoriaSelecionada.toLowerCase()
                    ? "block"
                    : "none";
        });
...
// ⬇️ Adicione este bloco aqui dentro da função populateCategories(), depois do código acima
document.querySelectorAll('.category').forEach(cat => {
    cat.addEventListener('click', () => {
        const categoriaSelecionada = cat.dataset.category;
        ...
        document.querySelectorAll('.highlight[data-category]').forEach(section => {
            ...
            if (categoriaSelecionada === "Todos") {
                section.style.display = "block";
                if (listaLivros) listaLivros.classList.remove("catalogo-grid");
            } else {
                if (section.dataset.category === categoriaSelecionada) {
                    section.style.display = "block";
                    if (listaLivros) listaLivros.classList.add("catalogo-grid");
                } else {
